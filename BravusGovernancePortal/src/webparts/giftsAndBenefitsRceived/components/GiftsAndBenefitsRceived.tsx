...
import type { IGiftsAndBenefitsRceivedProps } from './IGiftsAndBenefitsRceivedProps';
...
import { IGiftsAndBenefitsReceivedState } from './IGiftsAndBenefitsReceivedState';
...
import '../assets/GiftsAndBenefitsRceived.css';
import { GiftsAndBenefitsReceivedValidation } from '../../../frameworks/model/IGiftsAndBenefitsReceivedValidation';
import { IGiftItem } from '../../../frameworks/model/ISplistItem';
import * as moment from 'moment';
import { getSP } from '../../../pnpjsConfig';
import { Constant } from '../../../frameworks/constants/Constant';
import { Dropdown, IDropdownOption } from "@fluentui/react/lib/Dropdown";

export default class GiftsAndBenefitsReceived extends React.Component<IGiftsAndBenefitsRceivedProps, IGiftsAndBenefitsReceivedState> {
  ...
  public constructor(props: IGiftsAndBenefitsRceivedProps) {
    super(props);
    let validationConstant = new GiftsAndBenefitsReceivedValidation();
    this.state = {
      SplistItem: {
        fullname: '',
        designation: '',
  ...
  private async BindAllListdata(itemID: number) {
    try {
      ...
      const item: any = await this._listoperation.getItemById(this.props.context, Constant.SpListname, itemID, ["ID", "Title", "FullName", "Designation", "Department", "Company", "Date", "NotRecivedGifts", "ReceivedGiftsBenefits", "IterationDetails", "Signature", "SignatureDate", "Status"], [""]);
      console.log(item);
  ...
  handleDateChange = (field: keyof IGiftsAndBenefitsReceivedState["SplistItem"], date: any) => {
    this.setState(prevState => ({
      SplistItem: {
        ...prevState.SplistItem,
        [field]: date
  ...
  public async onResubmitClick() {
    try {
      ...
      await this._listoperation.updateItemInList(this.props.context, Constant.SpListname, this.CustID, listItem);
      // this.setState({ Isloader: false });
  ...
  handleSubmit = async () => {
    ...
    try {
      ...
      if (this.state.isEditMode && this.CustID) {
        isNewItem = true;
        itemId = this.CustID;
        await this._listoperation.updateItemInList(this.props.context, Constant.SpListname, Number(this.CustID), listItem);
      }
        ...
        await this._listoperation.updateItemInList(this.props.context, Constant.SpListname, this.CustID, itemsforUpdate);
      }
      else {
        isNewItem = true; // Mark as new item
        const response = await this._listoperation.addItemsToList(this.props.context, Constant.SpListname, listItem);
        itemId = response.Id;
      }
  ...
  private async triggerPowerAutomateFlow(itemId: number, itemData: any) {
    try {
      ...
      const requestBody = {
        itemId: itemId,
        ListName: Constant.SpListname,  // Passing the item ID to Power Automate
        ...itemData
      };
  ...
  handleSaveAsDraft = async () => {
    ...
    try {
      await this._listoperation.addItemsToList(this.props.context, Constant.SpListname, listItem);
    ...
    this.setState(prevState => ({
      SplistItem: {
        ...
        receivedGifts: [
          ...
          { dateGiven: null, recipientName: '', businessEntity: '', provider: '', description: '', value: '', retained: '', reason: '' }
        ]
      }
    }));
  };
  ...
  handleGiftChange = (index: number, field: keyof IGiftItem, value: any) => {
    const updatedGifts = [...this.state.SplistItem.receivedGifts];
  ...
  public render(): React.ReactElement<IGiftsAndBenefitsRceivedProps> {
    ...
    return (
      <div className="gift-form-container">
        {this.state.Isloader ?
          ...
          : undefined}
        <h2 className='gift-form-h2'>Gifts and benefits received Self-Declaration Form</h2>
        ...
        <div className="section-header">Section 2: Declaration of the receipt of Gifts & Benefits</div>
        <p className="declaration-text">I, [Your Name], hereby declare that:</p>
        <div className='checkboxWrapper'>
          <Checkbox
            label="I have not received any gifts or benefits"
          ...
          <table className="gift-table">
            <thead>
              <tr>
                ...
                <th>Recipient's business entity</th>
                <th>Name of person / organization providing gift or benefit</th>
                <th>Description of gift or benefit</th>
                <th>Value ($)</th>
                <th>Was gift or benefit retained by recipient or business?</th>
                <th>Reason(s) for accepting (what is the business benefit?)</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={index}>
                  ...
                  <td>
                    <TextField
                      ...
                      onChange={(_, value) => this.handleGiftChange(index, 'recipientName', value)}
                      disabled={this.state.status === "Submit" || this.state.status === "Approved" || this.state.status === "Pending with Compliance Group"}
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <TextField
                      value={gift.provider}
                      onChange={(_, value) => this.handleGiftChange(index, 'provider', value)}
                      disabled={this.state.status === "Submit" || this.state.status === "Approved" || this.state.status === "Pending with Compliance Group"}
                    />
                  </td>
                  <td>
                  ...
                  <td>
                    <TextField
                      value={gift.retained}
                      onChange={(_, value) => this.handleGiftChange(index, 'retained', value)}
                      disabled={this.state.status === "Submit" || this.state.status === "Approved" || this.state.status === "Pending with Compliance Group"}
                    />
                  </td>
                  <td>
        ...
        <p className="declaration-text">
          I confirm that the information provided above is true and complete to the best of my knowledge. I understand that failure to disclose a receipt of gifts or benefits may result in disciplinary action. I agree to notify [Company Name] of any changes in my circumstances related to the receipt of gifts or benefits.
        </p>
        ...
        <div className='datePickerWrapper'>
          <DatePicker label="Signature Date" onSelectDate={(date) => this.handleDateChange('signatureDate', date)} value={this.state.SplistItem.signatureDate} className="form-field" disabled={this.state.status === "Submit" || this.state.status === "Approved" || this.state.status === "Pending with Compliance Group"} isRequired={true} /><br></br>
